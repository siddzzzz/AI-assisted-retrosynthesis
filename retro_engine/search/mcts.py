...
from ..critic.steric_3d import Steric3DAnalyzer
from ..critic.stoichiometry import ReactionStoichiometryCalculator
from ..critic.precedent_matcher import PrecedentMatcher
from ..critic.byproduct_predictor import ByproductPurificationPredictor
...
class ReactionStepNode:
    ...
    explanation: str
    sterics: Dict[str, Any] = field(default_factory=dict)
    precedent: Dict[str, Any] = field(default_factory=dict)
    purification: Dict[str, Any] = field(default_factory=dict)
    stoichiometry: Dict[str, Any] = field(default_factory=dict)
...
class RetrosynthesisSearchTree:
    ...
    def plan_synthesis(
        ...
        def search_mol(
            ...
            # Terminating leaf check (only if not at root depth)
            ...
            for rule in rules:
                ...
                for reactants_smi, matched_rule in disconnections:
                    ...
                    # Researcher-centric deep calculations
                    sterics_info = Steric3DAnalyzer.evaluate_molecule_sterics(current_smi, matched_rule.reaction_class)
                    precedent_info = PrecedentMatcher.find_nearest_precedent(current_smi, matched_rule.reaction_class)
                    purification_info = ByproductPurificationPredictor.forecast_purification(matched_rule.reaction_class, current_smi)
                    stoichiometry_info = ReactionStoichiometryCalculator.calculate_step_stoichiometry(
                    ...
                    step_dict = {
                        ...
                        "sterics": sterics_info,
                        "precedent": precedent_info,
                        "purification": purification_info,
        ...
        for idx, route_steps in enumerate(raw_routes):
            ...
            for s_num, s_data in enumerate(annotated_steps, start=1):
                node = ReactionStepNode(
                    ...
                    sterics=s_data.get("sterics", {}),
                    precedent=s_data.get("precedent", {}),
                    purification=s_data.get("purification", {}),
                    stoichiometry=s_data.get("stoichiometry", {}),
            ...
            for r_smi in leaf_reactants:
                ...
                if info:
                    ...
                    starting_mats.append({
                        "smiles": r_smi,
                        "name": "Commodity Chemical Feedstock",
        ...
        # Sort plans: prioritize complete multi-step routes with high green score & cumulative yield
        dedup_plans.sort(
            key=lambda p: (
                p.metrics.total_steps * 5.0
